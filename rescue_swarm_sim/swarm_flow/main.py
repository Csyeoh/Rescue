...
class SwarmCombinedFlow:
    ...
    async def swarm_tick(self):
        ...
        # -> AI AMNESIA: Use a unique session_id per tick so context history resets to 0!
        fresh_session_id = f"dispatcher_tick_{tick_count}"
        
        await self.run_agent(dispatcher, "Analyze mission status and coordinate the swarm.", session_id=fresh_session_id)
